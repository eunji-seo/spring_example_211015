...
<body>
	<div class="container">
		...
		<button id="joinBtn" class="btn btn-success" type="submit">가입하기</button>
	</div>	
<script>
$(document).ready(function(){
	...
	$('#nameCheckBtn').on('click',function(){
		...
		// div 태그 안에 있는 내용을 모두 지운다
			$('#nameStatusArea').empty();
		
		// validation check
		if(name == ''){
			$('#nameStatusArea').append('<spen class="text-danger">이름이 비어있습니다.</span>');
			return;
		}
		...
		$.ajax({
			...
			, success: function(data){
				...
				if(data.is_duplication == true){
					$('#nameStatusArea').append('<spen class="text-danger"> 중복된 이름입니다.</span>');	
				}
